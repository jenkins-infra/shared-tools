...
** Terraform CLI to provision the infrastructure
*** Get the current production version from the latest published release of https://github.com/jenkins-infra/packer-images/blob/main/provisioning/tools-versions.yml
...
* The value of the environment is provided by the [`terraform()` pipeline library](https://github.com/jenkins-infra/pipeline-library/blob/master/vars/terraform.groovy) through the environment variable `TF_VAR_environment` (which may be optionally consumed by Terraform if an input value `environment` is defined in the projet).

* Each environment has a custom AzureRM Storage backend defined, by convention, in 3 partials configuration items:
** In the Terraform project: a backend of type `azurerm` in the file `backend.tf` with the attributes common to all environments
** In the Terraform project: a file: `./backend-configs/$(TF_VAR_environment)` with the non sensitive attributes specific to each environment
** In the environment: the environment variable `TF_BACKEND_ARM_CLIENT_SECRET` should hold the Service Principal secret value
*** On infra.ci.jenkins.io: provided through Jenkins credentials
*** On our admin machines: through the OS vault (Keychain access on macOS with `export TF_VAR_environment=production;export TF_BACKEND_ARM_CLIENT_SECRET="$(security find-generic-password -a jenkins-tf-<project name>-backend-arm-secret -w)"` for instance)
...
* `agentLabel`:  (Optional - Default: "jnlp-linux-arm64"), Agent use, providing a reproductible environment (fixed versions of Terraform, golang, etc.) from the ALLINONE: https://github.com/jenkins-infra/packer-images
...
terraform(
	stagingCredentials: [
		...
		string(variable: 'TF_BACKEND_ARM_CLIENT_SECRET', credentialsId:'staging-terraform-aws-backend-config-arm-secret'),
	],
	productionCredentials: [
		...
		string(variable: 'TF_BACKEND_ARM_CLIENT_SECRET', credentialsId:'production-terraform-aws-backend-config-arm-secret'),
	],
)
----
